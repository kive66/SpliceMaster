...
namespace SpliceMaster {
    ...
    public class MidiFile {
        ...
        private MidiFileReader file;
        private List<int> trackEndPos;    /** record the end position of every track including the track without note */
        private int ticksPerBar;          /** The number of ticks per bar */
        private List<List<int>> partNum;  /** indicate the verse, pre-chorus and chorus bar number */
        private int[] cutPos;             /** the cutting position of current midi file */
        private int loudness;             /** record the average loudness of the midifile */
        ...
        /** Get the quarternote (in pulses) of the song */
        public int Quarternote {
            get { return quarternote; }
        }

        /** Get the MidiFileReader of the MidiFile */
        public MidiFileReader File {
            get { return file; }
        }

        /** Get the ticks per bar of the MidiFile */
        public int TicksPerBar {
            get { return ticksPerBar; }
        }

        /** Get and set the partNum of the MidiFile */
        public List<List<int>> PartNum {
            get { return partNum; }
            set { partNum = value; }
        }

        /** Get and set the cutting position of the midifile */
        public int[] CutPos {
            get { return cutPos; }
            set { cutPos = value; }
        }
        ...
        public MidiFile(string filename) {
            file = new MidiFileReader(filename);
            parse(filename);
        }
        ...
        public void parse(string filename) {
            ...
            trackEndPos = new List<int>();
            partNum = new List<List<int>>();
            cutPos = new int[2];
            ...
            ticksPerBar = quarternote / (timesig.Denominator / 4) * timesig.Numerator;
            loudness = voice / voicenum;

            for (int tracknum = 0; tracknum < num_tracks; tracknum++) {
                MidiTrack track = new MidiTrack(events[tracknum], tracknum, timesig);
            ...
            for (int i= 0;i<tracks.Count;i++) {
                MidiNote last = tracks[i].Notes[tracks[i].Notes.Count - 1];
                tracks[i].TrackPulses=last.StartTime + last.Duration;
         ...
         */
        private List<MidiEvent> ReadTrack() {
            ...
            while (file.GetOffset() < trackend) {
                ...
                mevent.StartOffset = startoffset;
                ...
                if (eventflag >= EventNoteOn && eventflag < EventNoteOn + 16) {
                    ...
                    if (mevent.Velocity != 0) {
                        voice += (int)mevent.Velocity;
                        voicenum+=1;
                    }
                    ...
                    if (mevent.Metaevent == MetaEventTimeSignature) {
                        ...
                        mevent.KeySignature = mevent.Value[0];
                    }
        ...
        /** Find the position to cut the track */
        public int FindCutPosition(int trackStart, int ticks) {
            int currTicks = 0;
            file.SetOffset(trackStart);
            // ��λ����λ��
            file.ReadAscii(4);  // track chunk

            file.ReadInt();  // tracklen

            int eventflag = 0, nBytes;
            while (currTicks < ticks) {
                nBytes = 0;
                currTicks += file.ReadVarlen(ref nBytes);  // deltatime
                if (currTicks >= ticks)
                    return file.GetOffset() - nBytes;
                ...
                if (eventflag >= EventNoteOn && eventflag < EventNoteOn + 16) {
                    file.ReadByte();  // notenumber
                    file.ReadByte();  // velocity
                }
                else if (eventflag >= EventNoteOff && eventflag < EventNoteOff + 16) {
                    file.ReadByte();  // notenumber
                    file.ReadByte();  // velocity
                         ...
                         eventflag < EventKeyPressure + 16) {
                    file.ReadByte();  // motenumber
                    file.ReadByte();  // pressure
                }
                else if (eventflag >= EventControlChange &&
                         eventflag < EventControlChange + 16) {
                    file.ReadByte();  // controlNum
                    file.ReadByte();  // controlValue
                }
                else if (eventflag >= EventProgramChange &&
                         eventflag < EventProgramChange + 16)
                    file.ReadByte();  // instrument
                else if (eventflag >= EventChannelPressure &&
                         eventflag < EventChannelPressure + 16)
                    file.ReadByte();  // chanPressure
                else if (eventflag >= EventPitchBend &&
                         eventflag < EventPitchBend + 16)
                    file.ReadShort();  // pitchBend
                else if (eventflag == SysexEvent1) {
                    int metaLength = file.ReadVarlen();  // metaLength
                    file.ReadBytes(metaLength);  // value
                }
                else if (eventflag == SysexEvent2) {
                    int metaLength = file.ReadVarlen();  // metaLength
                    file.ReadBytes(metaLength);  // value
                    ...
                    int Metaevent = file.ReadByte();  // metaEvent
                    int Metalength = file.ReadVarlen();
                    file.ReadBytes(Metalength);  // value
                    if (Metaevent == MetaEventEndOfTrack)
                        break;
                }
                else
                    throw new MidiFileException("Unknown event " + eventflag, file.GetOffset() - 1);
            }
            // ���ؼ���λ��
        ...
        /*
             ...
             */
        public static void ShiftTime(List<MidiTrack> tracks, int amount) {
